...
okt = Okt()
# print(os.listdir('.')) # 작동하는 곳의 현 위치를 나타냄. # ! 혹시 누군가 사용한다면 유용하게 쓰일 기능입니다.(python과 ipynb의 기본경로가 다르다는 점 주의!) 
...
def recommend_from_hashtag(hashtags):
    ...
    #키워드에 따라 유사단어를 추천해주는 함수.
    x = model.wv.most_similar(positive = hashtags)
    temp = []
    for i in x:
        temp.append(i[0])
        if len(temp) == 10:
